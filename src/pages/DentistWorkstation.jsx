...
const DentistWorkstation = () => {
  ...
  const handleSaveNotes = async () => {
    ...
    setSaving(true);
    try {
      // First, get the state ID for 'completada' - Use a standard select to avoid 406 errors
      ...
      const estadoCompletada = estadosData?.find(e => e.nombre.toLowerCase() === 'completada');
      
      if (!estadoCompletada) {
        console.error("Available statuses:", estadosData);
        throw new Error("Status 'completada' not found in database. Available: " + (estadosData?.map(e => e.nombre).join(', ') || 'none'));
      }
      ...
      setAppointments(prev => prev.map(a => a.id === selectedApt.id ? { 
        ...a, 
        notas_medicas: notes, 
        estado_id: estadoCompletada.id,
        estados_cita: { nombre: 'completada' } 
      } : a));
      setSelectedApt(null);
    } catch (error) {
      console.error("Error saving notes:", error);
      addToast(error.message || t('dentist.error_saving'), 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto text-left pb-24 overflow-hidden">
      <LayoutGroup>
        {/* Navigation & View Switcher */}
        <header className="flex flex-col lg:flex-row lg:items-center justify-between gap-10 mb-16 px-2 lg:px-0">
          <div className="flex flex-col sm:flex-row sm:items-center gap-8">
            <h2 className="text-4xl sm:text-5xl font-black tracking-tight text-slate-900 min-w-[280px]">
            ...
            <div className="flex items-center bg-white border border-slate-100 rounded-3xl p-1.5 shadow-sm w-fit">
               <button onClick={() => navigateDate('prev')} className="p-4 hover:bg-slate-50 rounded-2xl text-slate-400 hover:text-primary transition-all">
                  <span className="material-symbols-outlined text-2xl">chevron_left</span>
               </button>
               <button onClick={() => setCurrentDate(new TZDate(new Date(), BOGOTA_TZ))} className="px-8 text-[11px] font-black uppercase tracking-[0.25em] text-slate-400 hover:text-primary transition-all">
                 {t('dentist.today_btn')}
               </button>
               <button onClick={() => navigateDate('next')} className="p-4 hover:bg-slate-50 rounded-2xl text-slate-400 hover:text-primary transition-all">
                  <span className="material-symbols-outlined text-2xl">chevron_right</span>
               </button>
            </div>
          </div>

          <div className="flex items-center gap-2 bg-slate-100/50 p-1.5 rounded-[1.8rem] self-start lg:self-center overflow-x-auto no-scrollbar max-w-full shadow-inner">
            {['today', 'weekly', 'monthly'].map(mode => (
              <button 
                key={mode}
                onClick={() => setViewMode(mode)}
                className={`relative px-8 sm:px-10 py-3.5 rounded-[1.2rem] text-[10px] sm:text-[11px] font-black uppercase tracking-[0.25em] transition-all z-10 whitespace-nowrap ${viewMode === mode ? 'text-white' : 'text-slate-400 hover:text-slate-600'}`}
              >
                {t(`dentist.tabs.${mode}`)}
                {viewMode === mode && (
                  <motion.div 
                    layoutId="activeTab"
                    className="absolute inset-0 bg-primary rounded-[1.2rem] shadow-2xl shadow-primary/30 -z-10"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start px-2 lg:px-0">
          {/* Main View Area */}
                ...
                {viewMode === 'today' && (
                  <div className="space-y-12">
                    ...
                    {nextPatient && isToday(currentDate) && (
                      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-slate-900 rounded-[3rem] p-8 sm:p-12 text-white relative overflow-hidden group shadow-[0_40px_100px_rgba(15,23,42,0.3)]">
                        <div className="absolute top-0 right-0 p-16 opacity-5 group-hover:scale-125 transition-transform duration-1000 hidden sm:block">
                          <span className="material-symbols-outlined text-[12rem]">medical_information</span>
                        </div>
                        <div className="relative z-10 space-y-8 sm:space-y-10">
                          <div className="flex items-center gap-6">
                            <span className="px-6 py-2.5 bg-primary/20 text-primary border border-primary/30 rounded-full text-[11px] font-black uppercase tracking-widest">{t('dentist.next_patient')}</span>
                            <div className="h-2 w-2 rounded-full bg-primary animate-ping" />
                            <span className="text-slate-400 text-base font-bold tracking-tight">{format(parseISO(nextPatient.fecha_hora), 'HH:mm')}</span>
                          </div>
                          <h3 className="text-4xl sm:text-6xl font-black tracking-tight leading-none">{nextPatient.pacientes?.nombre} {nextPatient.pacientes?.apellido}</h3>
                          <div className="flex flex-col sm:flex-row sm:items-center gap-8 sm:gap-12 pt-8 sm:pt-10 border-t border-white/10">
                            <div>
                              <p className="text-[11px] font-black uppercase text-slate-500 tracking-widest mb-2">{t('dentist.reason')}</p>
                              <p className="text-xl font-bold text-white uppercase italic tracking-tight">{nextPatient.motivo_nombre || 'Consulta General'}</p>
                            </div>
                            <button 
                              onClick={() => handleOpenNotes(nextPatient)}
                              className="w-full sm:w-auto sm:ml-auto bg-primary text-white px-12 py-6 rounded-[1.8rem] font-black text-[12px] uppercase tracking-[0.25em] hover:bg-white hover:text-slate-900 transition-all shadow-3xl shadow-primary/30 active:scale-95"
                            >
                              {t('dentist.treat_now')}
                            </button>
                          </div>
                    ...
                    <div className="space-y-4">
                      ...
                      {loading ? (
                        ...
                        <div className="p-12 sm:p-24 bg-white rounded-[2rem] sm:rounded-[3.5rem] border border-dashed border-slate-100 text-center shadow-sm">
                          <div className="w-16 h-16 sm:w-20 sm:h-20 bg-slate-50 rounded-2xl sm:rounded-3xl flex items-center justify-center mx-auto mb-6">
                    ...
                    {loading ? (
                      ...
                      <div className="grid grid-cols-1 gap-4">
                        {eachDayOfInterval({
                          ...
                          return (
                            <div key={day.toString()} className="bg-white rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-8 border border-slate-100 shadow-sm hover:shadow-md transition-all group">
                               <div className="flex items-center justify-between mb-6">
                                  <div className="flex items-center gap-4">
                                     <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl flex items-center justify-center font-black text-sm shadow-sm transition-all ${isToday(day) ? 'bg-primary text-white shadow-primary/20 scale-110' : 'bg-slate-50 text-slate-400 group-hover:bg-slate-100'}`}>
                                        {format(day, 'd')}
                                     </div>
                                     <h4 className={`text-lg sm:text-xl font-black uppercase tracking-tight ${isToday(day) ? 'text-primary' : 'text-slate-900'}`}>{format(day, 'EEEE', { locale })}</h4>
                                  </div>
                                  <span className="text-[10px] font-black uppercase text-slate-300 tracking-[0.2em] hidden sm:inline">{dayApts.length} {t('dentist.stats.total_today').toLowerCase()}</span>
                               </div>
          ...
          {/* Clinical Editor Sidebar / Modal */}
          <div className="lg:col-span-4 lg:sticky lg:top-12 z-[100]">
            <ClinicalEditor 
...
const StatCard = ({ label, value, icon, color }) => {
  ...
  return (
    <div className="bg-white p-6 sm:p-8 rounded-[1.5rem] sm:rounded-[2.5rem] border border-slate-100 shadow-sm flex items-center gap-4 sm:gap-6 group hover:shadow-xl hover:border-primary/10 transition-all duration-500">
      <div className={`w-12 h-12 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl flex items-center justify-center transition-all duration-500 group-hover:scale-110 group-hover:rotate-6 shrink-0 ${colors[color]}`}>
        <span className="material-symbols-outlined text-2xl sm:text-3xl">{icon}</span>
      </div>
      <div className="text-left">
        <p className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tighter leading-none mb-1">{value}</p>
        <p className="text-[9px] sm:text-[10px] font-black uppercase text-slate-400 tracking-[0.2em]">{label}</p>
      </div>
    </div>
  );
};
...
const AppointmentCard = ({ apt, isActive, onAction, t }) => (
  <motion.div 
    layout
    className={`p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] border transition-all duration-700 flex flex-col sm:flex-row sm:items-center justify-between gap-6 group ${isActive ? 'bg-primary/5 border-primary/30 shadow-2xl' : 'bg-white border-slate-100 hover:border-primary/20 shadow-sm hover:shadow-xl'}`}
  >
    <div className="flex items-center gap-4 sm:gap-8">
      <div className="text-center min-w-[60px] sm:min-w-[70px]">
        <p className="text-xl sm:text-2xl font-black text-slate-900 tracking-tighter">{format(parseISO(apt.fecha_hora), 'HH:mm')}</p>
        <p className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('dentist.time')}</p>
      </div>
      <div className="h-10 sm:h-12 w-[1px] bg-slate-100" />
      <div className="text-left min-w-0">
        <h4 className="text-lg sm:text-xl font-black text-slate-900 tracking-tight leading-tight group-hover:text-primary transition-colors truncate">{apt.pacientes?.nombre} {apt.pacientes?.apellido}</h4>
        <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-2">
          <span className={`px-3 py-1 sm:px-4 sm:py-1.5 rounded-full text-[8px] sm:text-[9px] font-black uppercase tracking-widest shadow-sm ${apt.estados_cita.nombre === 'completada' ? 'bg-green-100 text-green-600' : 'bg-primary/10 text-primary'}`}>
            {t(`appointments.status.${apt.estados_cita.nombre}`)}
          </span>
          <p className="text-[9px] sm:text-[10px] font-bold text-slate-300 uppercase tracking-widest truncate">ID: {apt.pacientes?.documento_id}</p>
        </div>
      </div>
    </div>
    <button 
      onClick={onAction}
      className={`w-full sm:w-auto px-8 sm:px-10 py-3.5 sm:py-4 rounded-xl sm:rounded-2xl font-black text-[9px] sm:text-[10px] uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-3 active:scale-95 ${apt.estados_cita.nombre === 'completada' ? 'bg-slate-100 text-slate-400' : 'bg-slate-900 text-white hover:bg-primary shadow-xl shadow-slate-900/10'}`}
    >
      <span className="material-symbols-outlined text-lg sm:text-xl">{apt.estados_cita.nombre === 'completada' ? 'history_edu' : 'edit_square'}</span>
    ...
    {selectedApt ? (
      <>
        ...
        <motion.div 
            ...
            className="fixed inset-x-4 bottom-4 lg:relative lg:inset-auto bg-white rounded-[2rem] sm:rounded-[3rem] shadow-[0_32px_80px_rgba(0,0,0,0.15)] border border-slate-100 p-6 sm:p-10 z-[100] max-h-[90vh] overflow-y-auto no-scrollbar group"
        >
            ...
            <button onClick={onCancel} className="ml-auto lg:hidden p-2 text-slate-400 hover:text-slate-600">
      ...
      <div className="hidden lg:flex bg-slate-50 rounded-[2.5rem] sm:rounded-[3.5rem] border-2 border-dashed border-slate-200 p-10 sm:p-16 text-center flex-col items-center justify-center min-h-[400px] sm:min-h-[600px] group">
